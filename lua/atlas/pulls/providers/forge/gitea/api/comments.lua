...
local reviews = require("atlas.pulls.providers.forge.gitea.api.reviews")
...
function M.add(pr, content, opts, on_done)
	...
	if parent and parent.inline then
		if opts and opts.pending == true then
			on_done(nil, "Gitea does not support pending replies to review comments")
			return nil
		end
